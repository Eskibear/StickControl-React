...
export default function Metronome() {
  ...
  const scheduler = useCallback(() => {
    ...
    while (nextNoteTimeRef.current < ctx.currentTime + scheduleAheadTime) {
      ...
      if (s.reps.selected && cnt > 0 && cnt % (s.reps.count * info.totalNotes) === 0) {
        const nextPat = (currentPatternRef.current + 1) % patterns.length;
  ...
  }, [dispatch, patterns, playClickAtTime]);
  ...
  const handleChangePattern = (e) => {
    ...
    val = Math.min(val, patterns.length);
    dispatch({ type: 'SET_CURRENT_PATTERN', value: val - 1 });
  };
  ...
  const patternSelect = currentPattern + 1;
